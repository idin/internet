...
class WikipediaPage:
	...
	def _load_from_id(self):
		...
		try:
			print(self.title, self.pensieve['page'], type(self.pensieve['page']))
			self.pensieve.store(key='url', precursors=['page'], function=lambda x: x['fullurl'], evaluate=False)
		except TypeError as e:
			print(self.pensieve['page'], type(self.pensieve['page']))
			raise e
	...
	def _load_the_rest(self):
		...
		self.pensieve['info_box_dictionary'] = lambda info_box: info_box.dictionary.copy()
...
# ToDo split the page into sections between headers
#